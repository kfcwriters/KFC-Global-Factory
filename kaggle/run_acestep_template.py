#!/usr/bin/env python3
"""
run_acestep_template.py — TEMPLATE, not run directly.
kaggle_music_gen.py fills in __PARAMS_JSON__ with real data and writes
the result to run_acestep.py before every kernel push. This guarantees
the kernel sees exactly the params we intend — no separate file, no
dataset attachment, no caching ambiguity (which caused earlier bugs).
"""
import os
...
# ── Params are baked directly into this script's source at push time ─────────
...
print(f"Lyrics preview: {params['lyrics'][:150]}")
