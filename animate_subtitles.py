...
class TeachingMasterclass(Scene):
    def construct(self):
        ...
        if os.path.exists('lecture_script.txt'):
            ...
            script_text = "Script missing."
        
        # 3. SEGMENTATION: 40 words per slide for clarity
        ...
        for i, chunk in enumerate(chunks):
            # HEADER - Scaled down slightly to fit 1080p frame
            header = Text(f"PHD MASTERCLASS | PART {i+1}", color=YELLOW, weight=BOLD).scale(1.2).to_edge(UP, buff=0.5)
            
            # THE FONT FIX (GOLDEN RATIO):
            # - wrap width=40 ensures text doesn't hit the left/right edges
            # - font_size=48 is huge but stays inside the 1080p frame
