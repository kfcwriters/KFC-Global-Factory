...
Runs weekly via GitHub Actions. Each run:
  1. Selects a new product from the catalog
  2. Generates a professional PDF
  3. Publishes it to Etsy as a digital download listing
...
def run():
    ...
    if not ETSY_OAUTH_TOKEN:
        raise EnvironmentError(
            "ETSY_OAUTH_TOKEN not set. "
            "This is the OAuth access token (different from the API keystring). "
            "See setup instructions for how to get this."
        )
    ...
    print("📋  Step 1/3 — Selecting product for this week ...")
    ...
    with tempfile.TemporaryDirectory(prefix="etsy_") as tmp:
        ...
        url = publish_product(ETSY_API_KEY, ETSY_OAUTH_TOKEN, product, pdf_path)
